...
def processState(s):
    return np.reshape(cv2.resize(s, (84, 90))[1:85, :, :]/255.0, [21168,])

def processState_partial(s):
    return zeros(21168) if np.random.random() > 0.5 else processState(s)
...
def saveToCenter(i,rList,jList,bufferArray,summaryLength,h_size,sess,mainQN,time_per_step):
    with open('./Center/log.csv', 'a') as myfile:
        ...
        for idx,z in enumerate(np.vstack(bufferArray[:,0])):
            img,state_display = sess.run([mainQN.salience,mainQN.rnn_state],\
                feed_dict={mainQN.scalarInput:np.reshape(bufferArray[idx,0],[1,21168]),\
    ...
    with open('./Center/frames/log'+str(i)+'.csv','w') as myfile:
        ...
        a, v = sess.run([mainQN.Advantage,mainQN.Value],\
            feed_dict={mainQN.scalarInput:np.vstack(bufferArray[:,0]),mainQN.trainLength:len(bufferArray),mainQN.state_in:state_train,mainQN.batch_size:1})
