...
from DQN import DuelQ
from DDQN import DeepQ
import time
    ...
    def train(self, steps_todo):
        ...
        while step_num < steps_todo:
            ...
            for i in range(NUM_FRAMES):
                s, r, done, _ = self.env.step(predict_movement)
                reward += r;
            ...
            if done:
                print("[{}/{}]Lived with maximum time ".format(step_num,steps_todo), alive_frame)
                print("[{}/{}]Earned a total of reward equal to ".format(step_num,steps_todo), total_reward)
                self.env.reset()
                alive_frame = 0
                total_reward = 0
            ...
            if self.replay_buffer.size() > MIN_OBSERVATION:
                ...
                self.deep_q.train(s_batch, a_batch, r_batch, d_batch, s2_batch, step_num)
